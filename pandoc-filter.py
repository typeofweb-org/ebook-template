...
def justify(elem: Element, format: str, token: str) -> Element:
    ...
    if (format == 'html') or is_epub(format):
        ...
        new_children = (
            (
                [
                    RawInline(r'\noindent ~►\hspace{1em}', 'latex')
                ] if token == DIGRESSION else [
...
def digression_justify_paragraph(elem: Element, format: str) -> Element:
    """
    Digression-justify the specified element.
    ...
    Returns: the possibly updated element
    """
    return justify(elem, format, DIGRESSION)
...
def transform(elem: Element, doc: Doc) -> Element:
    ...
    elif paragraph_starts_with_child(elem, DIGRESSION):
        return digression_justify_paragraph(elem, doc.format)
